...
@Slf4j
public class FlinkBatchProcessorUnOptimized {
...
//            BatchAPISource.endBench(http, MainController.benchId);
//        } catch (IOException e) {
//            System.err.println("Failed to /api/end bench: " + e.getMessage());
//        }
    }
    ...
    public static class TileProcessor extends KeyedProcessFunction<Integer, Map<String, Object>, ProcessedResult> {
        ...
        private transient Map<Integer, TiffProcessorUnOptimized.SlidingCloseMeanCache> cacheMap;
        ...
        public void processElement(
            ...
            Integer layerId =  (Integer) data.get("layer");
...
//            log.info("Processing tile: tileId={}, batchId={}, printId={}", tileId, batchId, printId);
            ...
            System.out.println("[Subtask " + subtaskIndex + " Processing tile: tileId=" +tileId + ", layerId=" + layerId + " , batchId=" + batchId);
            ...
            try {
                ...
                BufferedImage newImage = TiffProcessorUnOptimized.readTiff(tiffData);
                ...
                // get cache
                TiffProcessorUnOptimized.SlidingCloseMeanCache cache =
                        cacheMap.computeIfAbsent(tileId, id -> new TiffProcessorUnOptimized.SlidingCloseMeanCache(
                                newImage.getWidth(), newImage.getHeight()));

                currentWindow.addLast(newImage);

                if (currentWindow.size() > WINDOW_SIZE) {
                    currentWindow.removeFirst(); // maintain sliding window
                }

                if (currentWindow.size() == WINDOW_SIZE) {
                    cache.initialize(currentWindow);
                }
                ...
                int saturated = 0;
                List<TiffProcessorUnOptimized.Centroid> centroids = Collections.emptyList();
                ...
                if (!currentWindow.isEmpty()) {
                    ...
                    if (currentWindow.size() >= WINDOW_SIZE) {
                        ...
                        centroids = TiffProcessorUnOptimized.processWindow(new ArrayList<>(currentWindow), cache, batchId);
                ...
                for (TiffProcessorUnOptimized.Centroid c : centroids) {
