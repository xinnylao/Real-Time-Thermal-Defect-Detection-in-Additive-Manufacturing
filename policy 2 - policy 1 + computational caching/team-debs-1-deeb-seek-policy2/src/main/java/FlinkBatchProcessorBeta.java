...
@Slf4j
public class FlinkBatchProcessorBeta {
...
//                        .assignTimestampsAndWatermarks(wmStrategy);
    ...
    public static class TileProcessor extends KeyedProcessFunction<Integer, Map<String, Object>, ProcessedResult> {
        ...
        public void processElement(
            ...
            try {
                ...
                BufferedImage newImage = TiffProcessorBeta.readTiff(tiffData);
                ...
                int saturated = 0;
                List<TiffProcessorBeta.Centroid> centroids = Collections.emptyList();

                if (!currentWindow.isEmpty()) {
                    BufferedImage currentImage = currentWindow.getLast();
                    saturated = TiffProcessorBeta.countSaturatedPixels(currentImage);

                    // only process full window of size 3 for centroids
                    if (currentWindow.size() >= WINDOW_SIZE) {
                        centroids = TiffProcessorBeta.processWindow(new ArrayList<>(currentWindow), batchId);
                    } else {
                ...
                for (TiffProcessorBeta.Centroid c : centroids) {
    ...
    public static void uploadResult(ProcessedResult result, String benchId) {
        try (CloseableHttpClient httpClient = HttpClientBuilder.create().build()) {
            ...
            System.err.println("Error uploading result: " + e.getMessage());
        }
    }
